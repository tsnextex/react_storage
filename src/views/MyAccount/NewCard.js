...
import {
  AddInitialPaymentMethod,
  AddNewPaymentMethod,
  AddNewPaymentMethodSiteLink
...
export const NewCard = props => {
  ...
  const handleSubmitPaymentMethod = () => {
    ...
    // Payment Profile missing
    // Step 1: post account profile
    // Step 2: post account credit card site link
    // gqlMutate(client,
    //   AddNewPaymentMethod(
    //     141,
    //     currentUser.user,
    ...
    //     } else addToSiteLink(res);
    //   });


    // Payment Profile exists
    // Step 1: post account credit card
    // Step 2: post account credit card site link
    gqlMutate(client,
      AddNewPaymentMethod(
        141,
  ...
  const addToSiteLink = res => {
    ...
    gqlMutate(client,
      ...
      .then(res => {
        ...
        console.log('Add to sitelink res');
        if (res.status === 'error') {
